...
void Turntable_3_Sensor_6(int val)
{
	...
	char result;
	if(Sensor_6_inhand == 0 && _State_Moudle.State_TurnPlate_3_Module == State_NoBusy && Get_Trak_State() == 1 && GetTurntableRespond() == 1)
	{
...
void Turntable_3_Sensor_5(int val)
{
	...
	message _message;
	if(Sensor_5_inhand == 0 && _State_Moudle.State_TurnPlate_3_Module == State_NoBusy && Get_Trak_State() == 1 && GetTurntableRespond() == 1)
	{
		if(BIT(val, 4) == 0)//表示传感器5触发过
		{
			if(BIT(val, 5) == 1)//传感器6优先
			{
				...
				{
					turntable_3_release = 1;
					if(Read_RFID(fd_RS485_index_3, RFID_4_ADDR, TRACK_OFFSET_OFFSET) != 0x08)
					{
						//TODO 报错
					}
				}
				else if(Get_Device_Mode() == 2 || Get_Device_Mode() == 6)
				{
					if((result = Read_RFID_Barcode2(fd_RS485_index_3, RFID_4_ADDR, &_message)) == 0)
					{
						...
						else
						{
							...
							{
								if(writeTraInfo(fd_RS485_index_3, RFID_4_ADDR, &_message, 18, NULL) != 0)
								{
									//TODO 报错
									return;
								}
							...
							else
							{
								...
								if(Read_RFID(fd_RS485_index_3, RFID_4_ADDR, _message._04_offset) == 0x08)
								{
...
char IsTurnplateRelease()
{
	...
	int dataLen = 0;
	char data[1000];
	if(Read_RFID(fd_RS485_index_3, RFID_4_ADDR, TRACK_OFFSET_OFFSET) == 0x08)//读轨迹信息的偏移扇区，固定从偏移0x04扇区读
	{
		if(PackByte(_ControlBoard[fd_RS485_index_3].Motor[RFID_4_ADDR].point + 12) == 0x7E)
		{
			...
			if(Read_RFID(fd_RS485_index_3, RFID_4_ADDR, _message._04_offset + 4) == 0x08)//读小车存储的条码
			{
				...
				else
				{
					...
					{
						if(Read_RFID(fd_RS485_index_3, RFID_4_ADDR, _message._04_offset + 4 + i * 4) == 0x08)//读小车存储的条码
						{
							for(j = 0;j < 91;j++)
							{
		...
		else
		{
			...
			if(Write_RFID(fd_RS485_index_3, RFID_4_ADDR, TRACK_OFFSET_OFFSET, buf) == 0x08)
			{
				return 1;
			}
			else
